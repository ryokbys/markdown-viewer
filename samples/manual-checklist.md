...
- [ ] `Export PDF` ボタンが右上に表示される
- [ ] 保存先とファイル名を選べる
- [ ] 現在の theme / font size / text width が反映された PDF が出力される
- [ ] `sepia` や `ocean` の背景が PDF にも反映される
- [ ] 出力後の PDF が Preview などで開ける
...
6. `Export PDF` で保存確認
7. `samples/demo.md` を外部エディタで編集して自動更新確認
